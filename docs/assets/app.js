...
function render(){
  ...
  const buys=[...active].sort((a,b)=>b[key].gross_buy_twd-a[key].gross_buy_twd);
  const sells=[...active].sort((a,b)=>b[key].gross_sell_twd-a[key].gross_sell_twd);
  ...
  document.querySelector("#buyRows").innerHTML=buys.map(s=>{const p=s[key];return `<tr><td>${stockCell(s)}</td><td class="positive">${fmtNum(p.buy_lots)}</td><td class="positive">${fmtMoney(p.gross_buy_twd)}</td></tr>`}).join("");
  document.querySelector("#sellRows").innerHTML=sells.map(s=>{const p=s[key];return `<tr><td>${stockCell(s)}</td><td class="negative">${fmtNum(p.sell_lots)}</td><td class="negative">${fmtMoney(p.gross_sell_twd)}</td></tr>`}).join("");
  ...
  document.querySelector("#buyLeaderFlow").textContent=topBuy?fmtMoney(topBuy[key].gross_buy_twd):"—";
  document.querySelector("#sellLeader").textContent=topSell?`${topSell.stock_id} ${topSell.stock_name}`:"—";
  document.querySelector("#sellLeaderFlow").textContent=topSell?fmtMoney(topSell[key].gross_sell_twd):"—";
}
